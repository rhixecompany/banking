...
import { openFileModal, setupModal } from "../modal";
  ...
  if (currentFilters.extensions.length > 0) {
    results = results.filter((item) => {
      ...
      return item.extensions?.some((ext) =>
        currentFilters.extensions.includes(ext),
      );
    });
  }
...
export async function initInstructionsPage(): Promise<void> {
  ...
  extensionSelect = createChoices("#filter-extension", {
    placeholderValue: "All Extensions",
  });
  extensionSelect.setChoices(
    data.filters.extensions.map((e) => ({ value: e, label: e })),
    "value",
    "label",
    true,
  );
  document
    .getElementById("filter-extension")
