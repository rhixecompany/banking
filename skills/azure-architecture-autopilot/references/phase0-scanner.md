...
- **Multiple RGs** → Repeat ask_user to add RGs one at a time. Stop when the user says "that's enough." Alternatively, the user can enter multiple RGs separated by commas (e.g., `rg-prod, rg-dev, rg-network`)
...
**Single subscription + single RG**: Same as before (VNet boundary only) **Multiple RGs (same subscription)**: Dashed boundary per RG **Multiple subscriptions**: Two-level boundary of Subscription > RG
...
When the user requests modifications, transition to Phase 1 (phase1-advisor.md). This is the **Path B entry point**, using the existing scan results as the baseline.
