...
- **CSV Structure Checks**
  ...
  - If no header row exists, assume the implicit column order:
    ...
    ```
  - Refer to the example input file: [`assets/example/01-user-input-rfc8805-feed.csv`](assets/example/01-user-input-rfc8805-feed.csv)
...
**MCP response key → JSON key mapping**: | MCP API response key | JSON key | |----------------------|----------------------------| | `placeName` | `Name` | | `countryCode` | `CountryCode` | | `stateCode` | `RegionCode` | | `placeType` | `PlaceType` | | `h3Cells` | `H3Cells` | | `boundingBox` | `BoundingBox` |
