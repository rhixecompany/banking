...
export const getMissingContributors = () => {
  try {
    ...
    if (
      details.toLowerCase().includes("bad credentials") ||
      details.includes("401")
    ) {
      console.error(
        "❌ all-contributors authentication failed (Bad credentials / 401).",
      );
      console.error(
        "💡 Set a valid token in PRIVATE_TOKEN (all-contributors-cli) and/or GH_TOKEN (gh CLI).",
      );
      console.error(
        "💡 In GitHub Actions, you can usually use: secrets.GITHUB_TOKEN",
      );
      throw new Error("contributors:check failed due to invalid credentials");
    }

    console.error(
      "❌ Error checking for missing contributors:",
 ...
 */
export const fetchContributorMergedPrs = (
  username,
  ...
  try {
    ...
    return prs.filter((pr) => {
      const hasNonConfigFiles = pr.files.some(
        (file) => !isAutoGeneratedFile(file.path),
      );
 ...
 * @returns {object|null}
 */
export const generateContributorReport = (
  username,
  { includeAllFiles = false } = {},
) => {
...
const main = () => {
  try {
    ...
    for (const contributor of contributors) {
      const report = generateContributorReport(contributor, {
        includeAllFiles,
      });
