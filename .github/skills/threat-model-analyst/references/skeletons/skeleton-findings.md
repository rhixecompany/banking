...
> **⛔ Copy the template content below VERBATIM (excluding the outer code fence). Replace `[FILL]` placeholders. ALL 10 attribute rows are MANDATORY per finding. Organize by TIER, not by severity.** **⛔ DO NOT abbreviate attribute names. Use EXACT names: `SDL Bugbar Severity` (not `Severity`), `Exploitation Prerequisites` (not `Prerequisites`), `Exploitability Tier` (not `Tier`), `Remediation Effort` (not `Effort`), `CVSS 4.0` (not `CVSS Score`).** **⛔ DO NOT use bold inline headers (`**Description:**`). Use `#### Description` markdown h4 headings.** **⛔ Tier section headings MUST be: `## Tier 1 — Direct Exposure (No Prerequisites)`, NOT `## Tier 1 Findings`.**
...
[END-REPEAT] [CONDITIONAL-EMPTY: If no Tier 1 findings, include this line instead of the REPEAT block] _No Tier 1 findings identified for this repository._ [END-CONDITIONAL-EMPTY]
...
[END-REPEAT] [CONDITIONAL-EMPTY: If no Tier 2 findings, include this line instead of the REPEAT block] _No Tier 2 findings identified for this repository._ [END-CONDITIONAL-EMPTY]
...
[END-REPEAT] [CONDITIONAL-EMPTY: If no Tier 3 findings, include this line instead of the REPEAT block] _No Tier 3 findings identified for this repository._ [END-CONDITIONAL-EMPTY]
```
...
[REPEAT: one row per threat from ALL components in 2-stride-analysis.md] | [FILL: T##.X] | [FILL: FIND-## or —] | [FILL: ✅ Covered (FIND-XX) / ✅ Mitigated (FIND-XX) / 🔄 Mitigated by Platform] | [END-REPEAT]
