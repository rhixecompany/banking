...
- Tech stack, deployment model: update if changed, otherwise carry forward
  ...
  ⛔ **DEPLOYMENT CLASSIFICATION IS MANDATORY (even in incremental mode):** The `0.1-architecture.md` MUST contain:
...
- ZERO CVSS vectors may use `AV:N` unless component has `Reachability = External` For ALL classifications:
...
```markdown
| Baseline Report | `{baseline_folder}` | | Baseline Commit | `{baseline_sha}` (`{baseline_commit_date}` — run `git log -1 --format="%cs" {baseline_sha}`) | | Target Commit | `{target_sha}` (`{target_commit_date}` — run `git log -1 --format="%cs" {target_sha}`) | | Baseline Worktree | `{worktree_path}` | | Analysis Mode | `Incremental` |
```
...
<div class="metrics-bar">
  ...
  → {{new_count}} (±N) Findings: {{old_count}} → {{new_count}} (±N)
...
<!-- Section 8: Footer -->
<div class="footer">
  Model: {{model}} | Duration: {{duration}} Baseline:
  {{baseline_folder}} at {{baseline_sha}} Generated: {{timestamp}}
</div>
```
