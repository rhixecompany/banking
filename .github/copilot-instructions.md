...
Where Copilot should look first for cross-cutting context: package.json (scripts), app-config.ts, next.config.ts, AGENTS.md, actions/*, dal/*, scripts/verify-rules.ts.
...
- Server Actions for all mutations: placed under actions/*. Contract: validate with Zod, authenticate early, use dal/* for DB, return { ok: boolean, error?: string } and revalidate caches/tags where needed.
