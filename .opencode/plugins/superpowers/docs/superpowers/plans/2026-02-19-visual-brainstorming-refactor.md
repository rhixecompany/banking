...
ws3.send(
  JSON.stringify({ type: "click", choice: "a", text: "Option A" })
);
await sleep(300);
...
fs.writeFileSync(
  path.join(TEST_DIR, "new-screen.html"),
  "<h2>New screen</h2>"
);
await sleep(500);
assert(
...
```javascript
assert(
  !fullRes.body.includes("indicator-bar") ||
    fullDoc.includes("indicator-bar"),
...
assert(
  helperContent.includes("sendEvent"),
  "helper.js should define sendEvent"
);
assert(
