...
Prompt includes: "Before starting the server:
...
File to modify: [exact path] Pattern to follow: [reference to existing function/test] What to implement: [specific requirement] Verification: [exact command to run]
...
**Why this works:** Reduces token usage, increases focus, faster completion when appropriate.
...
**Why this works:** Catches bugs implementer can find themselves before handoff. Documented case: identified entrypoint bug through self-reflection.

**Trade-off:** Adds ~30 seconds per task, but catches issues before review.
...
**Why this works:** Explicit instruction prevents "file not found" issues.
...
**Why this works:** Ensures skills are actually used, not just exist.

**Trade-off:** Adds time to each task, but prevents entire classes of bugs.
...
**Why this works:** Reduces latency when implementer already knows the fix. Documented case: would have saved one round-trip for entrypoint bug.

**Trade-off:** Slightly more complex prompt, but faster end-to-end.
...
**Problem:** Adding all these requirements makes prompts overwhelming **Mitigation:**
...
**Problem:** Too much reflection/verification slows execution **Mitigation:**
...
**Problem:** Following checklist doesn't guarantee correctness **Mitigation:**
...
**Problem:** Different skills give conflicting advice **Mitigation:**
