...
chokidar.watch(SCREEN_FILE).on("change", () => {
  console.log(
    JSON.stringify({ type: "screen-updated", file: SCREEN_FILE })
  );
...
Run: `cd lib/brainstorm-server && npm install` Expected: Dependencies installed

**Step 4: Test server starts**

Run: `cd lib/brainstorm-server && timeout 3 node index.js || true` Expected: See JSON with `server-started` and port info
...
Run: `node -c lib/brainstorm-server/helper.js` Expected: No syntax errors
...
async function runTests() {
  ...
  try {
    ...
    assert(
      ...
      "Should output server-started"
    );
    assert(
      stdout.includes(TEST_PORT.toString()),
      "Should include port"
    );
    console.log("  PASS");
    ...
    assert(
      ...
      "Should include brainstorm content"
    );
    assert(
      res.body.includes("WebSocket"),
...
Run: `cd tests/brainstorm-server && npm install ws && node server.test.js` Expected: All tests pass
...
```html
<button onclick="brainstorm.choice('custom', {extra: 'data'})">
  Custom
</button>
```
...
Run: `grep -A5 "Visual Companion" skills/brainstorming/SKILL.md` Expected: Shows the new section
