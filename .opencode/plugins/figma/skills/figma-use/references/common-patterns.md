...
rect.resize(200, 100);
rect.fills = [
  { type: "SOLID", color: { r: 0.047, g: 0.549, b: 0.914 } }
];
rect.cornerRadius = 8;
...
```js
const collection =
  figma.variables.createVariableCollection("Theme/Colors");
// Rename the default mode
collection.renameMode(collection.modes[0].modeId, "Light");
const darkModeId = collection.addMode("Dark");
const lightModeId = collection.modes[0].modeId;

const bgVar = figma.variables.createVariable(
  "bg",
  collection,
  "COLOR"
);
...
const textVar = figma.variables.createVariable(
  "text",
  collection,
  "COLOR"
);
...
```js
const variable =
  await figma.variables.getVariableByIdAsync("VariableID:1:2");
const rect = figma.createRectangle();
...
for (const variant of variants) {
  ...
  // TEXT property — label
  const labelKey = comp.addComponentProperty(
    "Label",
    "TEXT",
    "Button"
  );
  ...
  const showIconKey = comp.addComponentProperty(
    "Show Icon",
    "BOOLEAN",
    false
  );
...
const compSet = await figma.importComponentSetByKeyAsync(
  "COMPONENT_SET_KEY"
);
const variant =
...
const colors = figma.variables.createVariableCollection(
  "Component/Colors"
);
...
textVar.setValueForMode(secondaryMode, {
  r: 0.1,
  g: 0.1,
  b: 0.1,
  a: 1
...
const collection =
  await figma.variables.getVariableCollectionByIdAsync(collId);
