...
const lightModeId = colorColl.modes.find(
  m => m.name === "Light"
).modeId;
const darkModeId = colorColl.modes.find(
  m => m.name === "Dark"
).modeId;
...
const fontSet = new Set(
  textStyles.map(s =>
    JSON.stringify({ family: s.family, style: s.style })
  )
);
...
const summary = collections.map(c => {
  ...
  return {
    ...
    missingCodeSyntax: vars.filter(
      v => !v.codeSyntax["WEB"] && !v.remote
    ).length,
