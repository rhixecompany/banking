...
Sentry.logger.trace("Starting authentication flow", {
  provider: "oauth"
});
...
Sentry.logger.info(
  Sentry.logger
    .fmt`User ${userId} purchased ${productName} for $${amount}`
);
...
Sentry.logger.error(
  Sentry.logger
    .fmt`Failed to load screen ${screenName}: ${error.message}`
);
```
...
function ProductScreen({ route }) {
  ...
  useEffect(() => {
    ...
    return () => {
      Sentry.logger.debug("Screen unmounted", {
        screen: "ProductScreen"
      });
    };
  }, []);
  ...
  const handlePurchase = async () => {
    Sentry.logger.info(
      Sentry.logger
        .fmt`User initiated purchase for product ${productId}`
    );
    try {
