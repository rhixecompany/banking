...
> **Minimum SDK:** `io.sentry:sentry-android` ≥ 7.0.0 for transaction-based profiling `io.sentry:sentry-android` ≥ 8.7.0 for continuous/UI profiling (recommended) **Requirement:** Profiling requires tracing to be enabled. Only transactions sampled for tracing can be profiled. **Platform requirement:** Android API 22+ (silently disabled on older devices)
