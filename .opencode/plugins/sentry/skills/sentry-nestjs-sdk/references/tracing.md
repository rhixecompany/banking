...
return Sentry.startSpan(
  ...
  async () => {
    ...
    await Sentry.startSpan(
      { name: "apply-discount", op: "function" },
      () => this.discountService.apply(cart)
    );

    return Sentry.startSpan(
      { name: "create-order", op: "db.query" },
...
Sentry.init({
  ...
  beforeSendSpan(span) {
    if (
      span.op === "db.query" &&
      span.description?.includes("password")
    ) {
...
> **Note:** Event spans always use `forceTransaction: true` — they appear as isolated root transactions, not child spans of the HTTP request that emitted the event.
...
export class ChatGateway {
  ...
  async handleMessage(
    @MessageBody() payload: { data: any; _sentry?: any }
  ) {
...
export class EmailProcessor {
  ...
  async handle(
    job: Job<{ userId: string; _sentry?: Record<string, string> }>
  ) {
...
> **`profileSessionSampleRate` is process-level** — decided once at startup, not per-request. Use `0.1` to profile 10% of pods in a fleet without overhead on the rest.
