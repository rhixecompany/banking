...
await Sentry.startSpan(
  { name: "checkout-flow", op: "function" },
  async () => {
...
Sentry.continueTrace(
  ...
  () => {
    return Sentry.startSpan(
      { name: "processJob", op: "queue.process" },
      () => doWork()
    );
  }
);
...
Sentry.startNewTrace(() => {
  return Sentry.startSpan({ name: "isolated-background-job" }, () =>
    doWork()
  );
});
```
...
Sentry.init({
  ignoreSpans: [
    ...
    span =>
      span.op === "db.query" && span.description?.includes("SELECT 1")
  ]
});
```
...
consumer.run({
  eachMessage: async ({ message }) => {
    Sentry.continueTrace(
      ...
      () =>
        Sentry.startSpan(
          { name: "Process order", op: "queue.process" },
...
> **NestJS has a dedicated skill: [`sentry-nestjs-sdk`](../sentry-nestjs-sdk/SKILL.md)** It covers `SentryModule.forRoot()`, `@SentryTraced` decorator for custom spans, `SentryTracingInterceptor`, and GraphQL resolver tracing.
...
Sentry.init({
  ...
  beforeSendSpan(span) {
    if (
      span.op === "http.client" &&
      span.data?.url?.includes("/internal")
    ) {
