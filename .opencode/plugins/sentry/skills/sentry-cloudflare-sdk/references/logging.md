...
Sentry.logger.debug("Cache lookup result: %s", [
  cacheHit ? "hit" : "miss"
]);
Sentry.logger.info("User %s authenticated successfully", [userId]);
Sentry.logger.warn("Rate limit approaching for key %s: %d/%d", [
  apiKey,
  current,
  limit
]);
Sentry.logger.error("Payment processing failed for order %s", [
  orderId
]);
Sentry.logger.fatal("Worker initialization failed: %s", [
  error.message
]);
```
