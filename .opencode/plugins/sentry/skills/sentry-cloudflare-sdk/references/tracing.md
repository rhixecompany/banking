...
> Minimum SDK: `@sentry/cloudflare` v8.0.0+ Streaming response span tracking: v10.x+ `propagateTraceparent`: v10.x+ OpenTelemetry compatibility tracer: v10.x+
...
Sentry.withSentry(
  (env: Env) => ({
    ...
    tracesSampler: samplingContext => {
      const url = samplingContext.attributes?.["url.full"] as
        | string
        | undefined;
