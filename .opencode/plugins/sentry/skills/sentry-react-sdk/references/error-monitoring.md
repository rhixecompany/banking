...
<Sentry.ErrorBoundary
  onMount={() =>
    analytics.track("error_boundary_mounted", {
      section: "dashboard"
    })
  }
  onUnmount={error => {
    ...
    >
      <Layout>
        <Sentry.ErrorBoundary
          fallback={<NavError />}
          beforeCapture={scope =>
            scope.setTag("section", "navigation")
          }
        >
          <Navigation />
        </Sentry.ErrorBoundary>
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      Sentry.captureException(err, {
        ...
        extra: {
          formFields: Object.fromEntries(new FormData(event.target))
        }
      });
    }
  }
...
Sentry.feedbackIntegration({
  ...
  onFormClose: () =>
    analytics.track("feedback_form_closed_without_submit"),
...
feedbackIntegration.attachTo(
  document.getElementById("help-menu-item")
);
```
