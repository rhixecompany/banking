...
await Sentry.startSpan(
  ...
  async () => {
    ...
    const [user, posts] = await Promise.all([
      ...
      Sentry.startSpan(
        { name: "fetchPosts", op: "http.client" },
...
socket.on("message", raw => {
  ...
  context.with(ctx, () => {
    Sentry.startSpan({ name: "ws.updateProfile" }, () =>
      handleMessage(msg)
    );
  });
});
...
Sentry.init({
  tracesSampler: ({ name, attributes, inheritOrSampleWith }) => {
    ...
    if (["/health", "/ping", "/readyz"].some(p => name.includes(p)))
      return 0;
...
Sentry.init({
  beforeSendSpan(span) {
    ...
    if (
      span.op === "http.client" &&
      span.description?.includes("/token")
    ) {
    ...
    span.data = {
      ...span.data,
      "deployment.region":
        import.meta.env.VITE_AWS_REGION ?? "unknown"
    };
