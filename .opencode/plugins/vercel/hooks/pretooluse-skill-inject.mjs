    ...
    const callbacks = {
      onPathGlobError(skill, p, err) {
        l.issue(
          ...
          { skill, pattern: p, error: String(err) },
        );
      },
  ...
  for (const skill of rankedSkills) {
    ...
    if (raw === null) {
      l.issue(
        ...
        { skillPath, error: "file not found or unreadable" },
      );
      continue;
    }
...
function run() {
  ...
  const seenEnv =
    typeof process.env.VERCEL_PLUGIN_SEEN_SKILLS === "string"
  ...
  const hasEnvDedup =
    !dedupOff && typeof process.env.VERCEL_PLUGIN_SEEN_SKILLS === "string";
