...
var FILE_MARKERS = [
  ...
  {
    file: "vercel.json",
    skills: ["vercel-cli", "deployments-cicd", "vercel-functions"],
  },
...
var GREENFIELD_DEFAULT_SKILLS = ["nextjs", "ai-sdk", "vercel-cli", "env-vars"];
...
async function main() {
  ...
  const userMessages = buildSessionStartProfilerUserMessages(
    greenfield,
    cliStatus,
  );
  ...
  try {
    if (platform === "claude-code") {
      for (const [key, value] of Object.entries(envVars)) {
        if (
          key === "VERCEL_PLUGIN_GREENFIELD" ||
          key === "VERCEL_PLUGIN_LIKELY_SKILLS"
        ) {
          continue;
        }
