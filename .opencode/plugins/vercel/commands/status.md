...
1. Check for `.vercel/project.json` in the current directory (or nearest parent).
   ...
   - **If not found**: print a clear message:
     > This project is not linked to Vercel. Run `vercel link` to connect it, then re-run `/status`. Stop here — remaining steps require a linked project. **Do NOT fall back to GitHub Actions (`gh run list`) or any other CI system.** The Vercel CLI is the only authoritative source for this command.
...
export async function POST(req: Request) {
  ...
  if (
    !signature ||
    !verifyDrainSignature(rawBody, signature, secret)
  ) {
