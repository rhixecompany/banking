...
function issueMentionsSyntheticTestSkill(issue: {
  ...
  return [issue.file, issue.message, issue.hint].some(
    (value) =>
      typeof value === "string" && value.includes(SYNTHETIC_TEST_SKILL_PREFIX),
  );
}
...
describe.serial("validate.ts", () => {
  test("repo has no non-synthetic validation errors", async () => {
    ...
    const unexpectedErrors = report.issues.filter(
      (issue: any) =>
        issue.severity === "error" && !issueMentionsSyntheticTestSkill(issue),
    );
  ...
  test("every issue has a check field matching its source check", async () => {
    ...
    const validCheckNames = new Set(
      report.checkResults.map((cr: any) => cr.name),
    );
  ...
  test("introducing an orphan skill causes validation failure", async () => {
    ...
    try {
      ...
      const orphanIssue = report.issues.find(
        (i: any) =>
          i.code === "ORPHAN_SKILL" &&
          i.message.includes("fake-orphan-test-skill"),
      );
  ...
  test("current commands pass command structure check", async () => {
    ...
    const cmdFiles = (await readdir(commandsDir)).filter(
  ...
  test("SKILL.md with 'vercel logs drain ls' in code fence fails CLI_BANNED_PATTERN", async () => {
    ...
    try {
      ...
      const issue = report.issues.find(
        (i: any) =>
          i.code === "CLI_BANNED_PATTERN" &&
          i.file?.includes("fake-banned-test-skill"),
      );
      expect(issue).toBeDefined();
      expect(issue.hint).toBeDefined();
  ...
  test("command.md with 'vercel integration deploy' in code fence fails CLI_BANNED_PATTERN", async () => {
    ...
    try {
      ...
      const issue = report.issues.find(
        (i: any) =>
          i.code === "CLI_BANNED_PATTERN" &&
          i.file?.includes("test-banned-cmd.md"),
      );
      expect(issue).toBeDefined();
      expect(issue.hint).toBeDefined();
    } finally {
  ...
  test("current repo has 0 CLI_BANNED_PATTERN issues", async () => {
    ...
    const bannedIssues = report.issues.filter(
      (i: any) => i.code === "CLI_BANNED_PATTERN",
    );
...
async function withTempSkill(
  ...
  const originalSkillMd = hadOriginal
  ...
  test("broken YAML frontmatter produces FM_INVALID_YAML", async () => {
    await withTempSkill(
      ...
      async () => {
        ...
        const issue = report.issues.find(
          (i: any) =>
            i.code === "FM_INVALID_YAML" &&
            i.message.includes("zzz-test-broken-yaml"),
        );
        expect(issue).toBeDefined();
        expect(issue.hint).toBeDefined();
      },
  ...
  test("string pathPatterns produces FM_PATHPATTERNS_TYPE", async () => {
    await withTempSkill(
      ...
      async () => {
        ...
        const issue = report.issues.find(
          (i: any) =>
            i.code === "FM_PATHPATTERNS_TYPE" &&
            i.message.includes("zzz-test-string-filepattern"),
        );
        expect(issue).toBeDefined();
  ...
  test("invalid bashPatterns regex produces PATTERN_BASH_COMPILE", async () => {
    await withTempSkill(
      ...
      async () => {
        ...
        const issue = report.issues.find(
          (i: any) =>
            i.code === "PATTERN_BASH_COMPILE" &&
            i.message.includes("zzz-test-bad-bash-regex"),
        );
        expect(issue).toBeDefined();
    ...
    await withTempSkill(
      ...
      async () => {
        ...
        const crashIssues = report.issues.filter(
          (i: any) =>
            i.message?.includes("zzz-test-nonstring-fp") &&
            i.code === "FM_INVALID_YAML",
        );
  ...
  test("SKILL.md with pathPatterns: [''] produces warning-level filtering (no crash)", async () => {
    await withTempSkill(
      ...
      async () => {
        ...
        const crashIssues = report.issues.filter(
          (i: any) =>
            i.message?.includes("zzz-test-empty-fp") &&
            i.code === "FM_INVALID_YAML",
        );
        expect(crashIssues).toEqual([]);
      },
    ...
    await withTempSkill(
      ...
      async () => {
        ...
        const issue = report.issues.find(
          (i: any) =>
            i.code === "FM_NO_NAME" &&
            i.message.includes("zzz-test-meta-name-mask"),
        );
        expect(issue).toBeDefined();
      },
    );
    ...
    await withTempSkill(
      ...
      async () => {
        ...
        const issue = report.issues.find(
          (i: any) =>
            i.code === "FM_PATHPATTERNS_TYPE" &&
            i.message.includes("zzz-test-structured-fp"),
        );
        expect(issue).toBeDefined();
  ...
  test("bashPatterns with nested quantifiers produces PATTERN_BASH_REDOS", async () => {
    await withTempSkill(
      ...
      async () => {
        ...
        const issue = report.issues.find(
          (i: any) =>
            i.code === "PATTERN_BASH_REDOS" &&
            i.message.includes("zzz-test-redos-pattern"),
        );
        expect(issue).toBeDefined();
...
describe.serial("validate.ts — catalog staleness", () => {
  test("generate-catalog.ts runs successfully", async () => {
    ...
    expect(code).toBe(0);
    expect(await exists(join(ROOT, "generated", "skill-catalog.md"))).toBe(
      true,
    );
  }, 30_000);
  ...
  test("generated catalog lists every non-synthetic skill from skills/ directory", async () => {
    ...
    const catalog = await readFile(
      join(ROOT, "generated", "skill-catalog.md"),
      "utf-8",
    );
    const skillsDir = join(ROOT, "skills");
    const dirs = await readdir(skillsDir);
    ...
    await withTempSkill(
      ...
      async () => {
        ...
        const issue = report.issues.find(
          (i: any) =>
            i.code === "CATALOG_STALE" &&
            i.message.includes("zzz-test-catalog-stale"),
        );
        expect(issue).toBeDefined();
