...
describe("bearer and token authorization", () => {
  test("masks Bearer tokens", () => {
    const result = redactCommand(
      "curl -H 'Authorization: Bearer eyJhbGciOiJIUzI1NiJ9.abc'",
    );
  ...
  test("masks 'token xxx' authorization style", () => {
    const result = redactCommand(
      "gh api -H 'Authorization: token ghp_abc123XYZ456'",
    );
...
describe("connection strings", () => {
  test("masks postgres connection string credentials", () => {
    const result = redactCommand(
      "psql postgres://admin:s3cret@db.example.com:5432/mydb",
    );
  ...
  test("masks redis connection string credentials", () => {
    const result = redactCommand(
      "redis-cli -u redis://default:hunter2@cache.example.com:6379",
    );
  ...
  test("masks mysql connection string credentials", () => {
    const result = redactCommand(
      "mysql mysql://root:pass123@localhost:3306/db",
    );
...
describe("JSON secret values", () => {
  test("masks token in JSON", () => {
    const result = redactCommand(
      'echo \'{"token": "sk_live_abc123", "name": "test"}\'',
    );
    expect(result).toContain('"token": "[REDACTED]"');
  ...
  test("masks secret and credential in JSON", () => {
    expect(redactCommand('{"secret": "s3cret"}')).toContain(
      '"secret": "[REDACTED]"',
    );
    expect(redactCommand('{"credential": "cred123"}')).toContain(
      '"credential": "[REDACTED]"',
    );
  });
});
...
describe("cookie header redaction", () => {
  ...
  test("masks Set-Cookie header values", () => {
    const result = redactCommand(
      "curl -v 'Set-Cookie: id=a3fWa; Path=/; HttpOnly'",
    );
...
describe("CLI flag redaction", () => {
  ...
  test("masks --credential flag value", () => {
    expect(redactCommand("tool --credential cred123")).toContain(
      "--credential [REDACTED]",
    );
...
describe("edge cases", () => {
  ...
  test("combined redaction: multiple pattern types in one command", () => {
    const cmd =
      "VERCEL_TOKEN=abc curl -H 'Authorization: Bearer eyJhbGciOiJI' --password s3cret https://x.co?key=k1";
