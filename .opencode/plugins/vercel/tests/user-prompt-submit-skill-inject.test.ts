...
describe("lexical prompt matching (VERCEL_PLUGIN_LEXICAL_PROMPT)", () => {
  const LEXICAL_ON = {
    VERCEL_PLUGIN_LEXICAL_PROMPT: "1",
  ...
  test("with flag on, lexical-only recall requires profiler evidence for the target skill", async () => {
    const { code, stdout } = await runHook("I want a conversation interface", {
      ...LEXICAL_ON,
      VERCEL_PLUGIN_LIKELY_SKILLS: "chat-sdk",
    });
    expect(code).toBe(0);
  ...
  test("lexical-only match respects 2-skill max and 8KB budget", async () => {
    ...
    const { code, stdout } = await runHook(
      ...
      {
        ...LEXICAL_ON,
        VERCEL_PLUGIN_LIKELY_SKILLS: "deployments-cicd,env-vars",
      },
    );
    expect(code).toBe(0);
    const result = JSON.parse(stdout);
