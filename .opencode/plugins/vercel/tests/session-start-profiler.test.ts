...
function parseLikelySkills(_envFileContent?: string): string[] {
  return readSessionFile(testSessionId, "likely-skills")
    .split(",")
    .filter(Boolean);
}
...
describe("session-start-profiler", () => {
  ...
  test("detects setup signals and enables setup mode when threshold is met", async () => {
    ...
    const bootstrapHints = parseCsvEnvVar(
      content,
      "VERCEL_PLUGIN_BOOTSTRAP_HINTS",
    );
    const resourceHints = parseCsvEnvVar(
      content,
  ...
  test("skips npm registry lookup when npm binary cannot be resolved", async () => {
    ...
    expect(result.stderr).toContain(
      "session-start-profiler:binary-resolution-skipped",
    );
...
describe("profileProject (unit)", () => {
  test("returns empty array for empty directory", async () => {
    ...
    const { profileProject } =
      await import("../hooks/session-start-profiler.mjs");
...
describe("logBrokenSkillFrontmatterSummary (unit)", () => {
  test("emits one summary warning when a skill has malformed frontmatter", async () => {
    const { logBrokenSkillFrontmatterSummary } =
      await import("../hooks/session-start-profiler.mjs");
    ...
    const logger = {
      ...
      debug: () => {},
      trace: () => {},
      isEnabled: (minLevel: string) =>
        minLevel === "summary" || minLevel === "off",
    };

    const message = logBrokenSkillFrontmatterSummary(pluginDir, logger as any);

    expect(message).toBe(
      "WARNING: 1 skills have broken frontmatter: broken-skill",
    );
    expect(summaries).toHaveLength(1);
    expect(summaries[0].event).toBe(
      "session-start-profiler:broken-skill-frontmatter",
    );
