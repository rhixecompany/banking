...
describe("patterns .mts/.mjs sync", () => {
  test("exported function names match", async () => {
    ...
    const srcFns = Object.keys(src)
      .filter((k) => typeof src[k] === "function")
      .sort();
    const compiledFns = Object.keys(compiled)
      .filter((k) => typeof compiled[k] === "function")
      .sort();
  ...
  test("globToRegex produces identical output", async () => {
    ...
    for (const input of inputs) {
      expect(compiled.globToRegex(input).source).toBe(
        src.globToRegex(input).source,
      );
    }
  });
  ...
  test("appendSeenSkill produces identical output", async () => {
    ...
    expect(compiled.appendSeenSkill("", "nextjs")).toBe(
      src.appendSeenSkill("", "nextjs"),
    );
    expect(compiled.appendSeenSkill("a,b", "c")).toBe(
      src.appendSeenSkill("a,b", "c"),
    );
    expect(compiled.appendSeenSkill(undefined, "x")).toBe(
      src.appendSeenSkill(undefined, "x"),
    );
  });
});
...
describe("pretooluse-skill-inject .mts/.mjs sync", () => {
  ...
  test("isDevServerCommand produces identical output", async () => {
    ...
    for (const input of inputs) {
      expect(compiled.isDevServerCommand(input)).toBe(
        src.isDevServerCommand(input),
      );
    }
  });
