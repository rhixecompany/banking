...
describe("SYNONYM_MAP bidirectional expansion", () => {
  ...
  test("database <-> postgres", () =>
    assertBidirectional("database", "postgres"));
  test("api <-> endpoint", () => assertBidirectional("api", "endpoint"));

  // New groups: cache/cdn/edge
  test("cache <-> cdn", () => assertBidirectional("cache", "cdn"));
  test("cache <-> revalidate", () =>
    assertBidirectional("cache", "revalidate"));
  test("cache <-> isr", () => assertBidirectional("cache", "isr"));

  // ssr/server-rendering
  test("ssr <-> server-rendering", () =>
    assertBidirectional("ssr", "server-rendering"));
  test("ssr <-> server-component", () =>
    assertBidirectional("ssr", "server-component"));
  ...
  test("analytics <-> tracking", () =>
    assertBidirectional("analytics", "tracking"));
  test("analytics <-> metrics", () =>
    assertBidirectional("analytics", "metrics"));
  test("analytics <-> observability", () =>
    assertBidirectional("analytics", "observability"));

  // middleware/interceptor
  test("middleware <-> interceptor", () =>
    assertBidirectional("middleware", "interceptor"));
  test("middleware <-> edge-middleware", () =>
    assertBidirectional("middleware", "edge-middleware"));

  // queue/background-jobs
  test("queue <-> background-jobs", () =>
    assertBidirectional("queue", "background-jobs"));
  ...
  test("monorepo <-> turborepo", () =>
    assertBidirectional("monorepo", "turborepo"));
  test("monorepo <-> workspace", () =>
    assertBidirectional("monorepo", "workspace"));

  // domain/dns
  test("domain <-> dns", () => assertBidirectional("domain", "dns"));
  test("domain <-> subdomain", () =>
    assertBidirectional("domain", "subdomain"));

  // redirect/rewrite
  test("redirect <-> rewrite", () =>
    assertBidirectional("redirect", "rewrite"));
  test("redirect <-> url-rewrite", () =>
    assertBidirectional("redirect", "url-rewrite"));
  ...
  test("error <-> error-handling", () =>
    assertBidirectional("error", "error-handling"));

  // webhook/callback
  test("webhook <-> callback", () =>
    assertBidirectional("webhook", "callback"));
  test("webhook <-> event-hook", () =>
    assertBidirectional("webhook", "event-hook"));

  // migration/schema-change
  test("migration <-> schema-change", () =>
    assertBidirectional("migration", "schema-change"));
  test("migration <-> database-migration", () =>
    assertBidirectional("migration", "database-migration"));

  // preview/staging
  test("preview <-> staging", () => assertBidirectional("preview", "staging"));
  test("preview <-> branch-deploy", () =>
    assertBidirectional("preview", "branch-deploy"));

  // serverless/lambda
  test("serverless <-> lambda", () =>
    assertBidirectional("serverless", "lambda"));
  test("serverless <-> edge-function", () =>
    assertBidirectional("serverless", "edge-function"));

  // rate-limit/throttle
  test("rate-limit <-> throttle", () =>
    assertBidirectional("rate-limit", "throttle"));
  test("rate-limit <-> quota", () =>
    assertBidirectional("rate-limit", "quota"));

  // feature-flag/toggle
  test("feature-flag <-> toggle", () =>
    assertBidirectional("feature-flag", "toggle"));
  test("feature-flag <-> experiment", () =>
    assertBidirectional("feature-flag", "experiment"));
  test("feature-flag <-> flags", () =>
    assertBidirectional("feature-flag", "flags"));
  ...
  test("seo <-> structured-data", () =>
    assertBidirectional("seo", "structured-data"));

  // perf/performance
  test("perf <-> performance", () =>
    assertBidirectional("perf", "performance"));
  ...
  test("realtime <-> streaming", () =>
    assertBidirectional("realtime", "streaming"));
  ...
  test("email <-> notification", () =>
    assertBidirectional("email", "notification"));
  ...
  test("payment <-> subscription", () =>
    assertBidirectional("payment", "subscription"));

  // ci/pipeline
  test("ci <-> pipeline", () => assertBidirectional("ci", "pipeline"));
  test("ci <-> github-actions", () =>
    assertBidirectional("ci", "github-actions"));
