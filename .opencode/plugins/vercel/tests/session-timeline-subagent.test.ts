...
async function runHookEnv(
  ...
  const payload = JSON.stringify({
    ...input,
    session_id: opts?.sessionId ?? testSession,
  });
  ...
  test("lead scaffold timeline dedups in the lead session but a fresh subagent gets its own nextjs injection", async () => {
    ...
    try {
      ...
      const leadScaffold = await runHookEnv(
        {
          tool_name: "Bash",
          tool_input: {
            command: "npx create-next-app@latest notion-clone --ts --app",
          },
        },
      ...
      const leadRead = await runHookEnv(
        {
          tool_name: "Read",
          tool_input: { file_path: "/Users/me/notion-clone/app/page.tsx" },
        },
      ...
      const dedupStrategyLine = debugLines.find(
        (line) => line.event === "dedup-strategy",
      );
