...
function copyTempHookRuntime(
  ...
  tempHooksDir: string,
  overrides: Partial<
    Record<(typeof TEMP_HOOK_RUNTIME_MODULES)[number], string>
  > = {},
): void {
...
function seedSeenSkills(skills: string[]): void {
  const seenFile = join(
    tmpdir(),
    `vercel-plugin-${testSession}-seen-skills.txt`,
  );
...
describe("pretooluse-skill-inject.mjs", () => {
  ...
  test("large multi-skill output is valid JSON with correct structure", async () => {
    ...
    expect(result.hookSpecificOutput.additionalContext.length).toBeGreaterThan(
      0,
    );
  ...
  test("match output uses correct hookSpecificOutput schema", async () => {
    ...
    expect(Object.keys(result.hookSpecificOutput)).toContain(
      "additionalContext",
    );
...
describe("skill-map from frontmatter", () => {
  test("buildSkillMap produces a valid skill map from SKILL.md files", async () => {
    const { buildSkillMap } =
      await import("../hooks/skill-map-frontmatter.mjs");
  ...
  test("every skill has at least one trigger pattern", async () => {
    ...
    for (const [skill, config] of Object.entries(map.skills) as [
      string,
      any,
    ][]) {
      ...
      if (pathCount === 0 && bashCount === 0 && importCount === 0)
...
async function runHookDebug(
  input: object,
): Promise<{ code: number; stdout: string; stderr: string }> {
  const payload = JSON.stringify({
    ...input,
    session_id: `dbg-${Date.now()}-${Math.random().toString(36).slice(2)}`,
  });
  const proc = Bun.spawn(["node", HOOK_SCRIPT], {
    stdin: "pipe",
    stdout: "pipe",
...
describe("issue events in debug mode", () => {
  ...
  test("SKILLMD_PARSE_FAIL issue emitted for malformed YAML frontmatter", async () => {
    ...
    const proc = Bun.spawn(
      ["node", join(tempHooksDir, "pretooluse-skill-inject.mjs")],
      {
        stdin: "pipe",
        stdout: "pipe",
        stderr: "pipe",
...
describe("setup mode bootstrap routing", () => {
  test("injects bootstrap on unmatched paths when setup mode is active", async () => {
    const { code, stdout } = await runHookEnv(
      {
        tool_name: "Read",
        tool_input: { file_path: "/project/random-not-matched.txt" },
      },
    ...
    expect(
      extractSkillInjection(result.hookSpecificOutput).matchedSkills,
    ).toContain("bootstrap");
    expect(
      extractSkillInjection(result.hookSpecificOutput).injectedSkills[0],
    ).toBe("bootstrap");
  });
  ...
  test("boosts bootstrap ahead of other skills in setup mode", async () => {
    ...
    expect(code).toBe(0);
    const result = JSON.parse(stdout);
    const injectedSkills = extractSkillInjection(
...
describe("seen-skills env file and dedup controls", () => {
  ...
  test("clearing file dedup state re-enables injection", async () => {
    ...
    expect(JSON.parse(injected).hookSpecificOutput.additionalContext).toContain(
      "Skill(nextjs)",
    );
  });
  ...
  test("debug mode logs dedup strategy for file, memory-only, and disabled", async () => {
    ...
    const fileLines = fileStderr
      .trim()
      .split("\n")
      .map((l: string) => JSON.parse(l));
    const fileStrategy = fileLines.find(
      (l: any) => l.event === "dedup-strategy",
    );
    ...
    const memoryOnlyLines = memoryOnlyStderr
      .trim()
      .split("\n")
      .map((l: string) => JSON.parse(l));
    const memoryOnlyStrategy = memoryOnlyLines.find(
      (l: any) => l.event === "dedup-strategy",
    );
    ...
    const disabledLines = disabledStderr
      .trim()
      .split("\n")
      .map((l: string) => JSON.parse(l));
    const disabledStrategy = disabledLines.find(
      (l: any) => l.event === "dedup-strategy",
    );
...
describe("glob regression", () => {
  ...
  test("vercel-deploy.yml workflow DOES trigger vercel-agent", async () => {
    const { code, stdout } = await runHook({
      tool_name: "Read",
      tool_input: {
        file_path: "/Users/me/project/.github/workflows/vercel-deploy.yml",
      },
    });
    expect(code).toBe(0);
    const result = JSON.parse(stdout);
  ...
  test("deploy-preview.yaml workflow DOES trigger vercel-agent", async () => {
    const { code, stdout } = await runHook({
      tool_name: "Read",
      tool_input: {
        file_path: "/Users/me/project/.github/workflows/deploy-preview.yaml",
      },
    });
    expect(code).toBe(0);
    const result = JSON.parse(stdout);
...
describe("priority ordering", () => {
  test("when 5 skills match, top 3 inject within cap", async () => {
    ...
    expect(
      getInjectedSkills(result.hookSpecificOutput).length,
...
describe("VERCEL_PLUGIN_DEBUG alias", () => {
  ...
  test("VERCEL_PLUGIN_DEBUG=1 produces identical event types as VERCEL_PLUGIN_HOOK_DEBUG=1", async () => {
    ...
    const { stderr: stderrNew } = await runHookEnv(input, {
      VERCEL_PLUGIN_DEBUG: "1",
      VERCEL_PLUGIN_HOOK_DEDUP: "off",
    });
    const { stderr: stderrOld } = await runHookEnv(input, {
      VERCEL_PLUGIN_HOOK_DEBUG: "1",
      VERCEL_PLUGIN_HOOK_DEDUP: "off",
    });
    const eventsNew = stderrNew
      .trim()
      .split("\n")
      .map((l: string) => JSON.parse(l).event);
    const eventsOld = stderrOld
      .trim()
      .split("\n")
      .map((l: string) => JSON.parse(l).event);
...
describe("match-reason logging", () => {
  ...
  test("matches-found reasons is empty object when no skills match", async () => {
    const { stderr } = await runHookEnv(
      {
        tool_name: "Read",
        tool_input: { file_path: "/project/totally-unrelated-file.xyz" },
      },
      { VERCEL_PLUGIN_DEBUG: "1" },
    );
    const lines = stderr
  ...
  test("matchType is basename when only basename matches", async () => {
    const { stderr } = await runHookEnv(
      {
        tool_name: "Read",
        tool_input: { file_path: "/some/deep/path/next.config.js" },
      },
      { VERCEL_PLUGIN_DEBUG: "1" },
    );
    const lines = stderr
    ...
    if (matchEvent.reasons["nextjs"]) {
      expect(["full", "basename", "suffix"]).toContain(
        matchEvent.reasons["nextjs"].matchType,
      );
    }
  });
});
...
describe("injection byte budget", () => {
  test("6000-byte budget still injects up to MAX_SKILLS for app/api/chat route", async () => {
    ...
    const { code, stdout } = await runHookEnv(
      ...
      {
        VERCEL_PLUGIN_HOOK_DEDUP: "off",
        VERCEL_PLUGIN_INJECTION_BUDGET: "6000",
      },
    );
    expect(code).toBe(0);
    const result = JSON.parse(stdout);
  ...
  test("large budget allows all matching skills up to MAX_SKILLS", async () => {
    ...
    const { code, stdout } = await runHookEnv(
      ...
      {
        VERCEL_PLUGIN_HOOK_DEDUP: "off",
        VERCEL_PLUGIN_INJECTION_BUDGET: "999999",
      },
    );
    expect(code).toBe(0);
    const result = JSON.parse(stdout);
  ...
  test("small skills can fill more than typical slots under generous budget", async () => {
    ...
    const { code, stdout } = await runHookEnv(
      ...
      {
        VERCEL_PLUGIN_HOOK_DEDUP: "off",
        VERCEL_PLUGIN_INJECTION_BUDGET: "15000",
      },
    );
    expect(code).toBe(0);
    const result = JSON.parse(stdout);
  ...
  test("invalid VERCEL_PLUGIN_INJECTION_BUDGET falls back to default", async () => {
    const { code, stdout } = await runHookEnv(
      ...
      {
        VERCEL_PLUGIN_HOOK_DEDUP: "off",
        VERCEL_PLUGIN_INJECTION_BUDGET: "not-a-number",
      },
    );
    expect(code).toBe(0);
    const result = JSON.parse(stdout);
  ...
  test("droppedByBudget appears in skillInjection metadata", async () => {
    ...
    expect(
      si.injectedSkills.length +
        si.droppedByCap.length +
        si.droppedByBudget.length,
    ).toBe(si.matchedSkills.length);
  });
});
...
describe("sectional injection (summary fallback)", () => {
  function createTempPlugin(
    ...
    const tempRoot = join(
      tmpdir(),
      `vp-test-sectional-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
    const tempHooksDir = join(tempRoot, "hooks");
    const tempSkillsDir = join(tempRoot, "skills");
    mkdirSync(tempSkillsDir, { recursive: true });
  ...
  test("full injection is kept when a summarized skill still fits within budget", async () => {
    ...
    const { tempHooksDir, cleanup } = createTempPlugin([
      ...
      {
        name: "skill-b",
        summary: "Short summary for skill-b.",
        body: bigBody,
        patterns: ["src/**"],
      },
    ]);
  ...
  test("total injected bytes stay within budget with summary fallback", async () => {
    ...
    const { code, stdout } = await runTempHook(
      ...
      {
        VERCEL_PLUGIN_HOOK_DEDUP: "off",
        VERCEL_PLUGIN_INJECTION_BUDGET: String(budget),
      },
    );
    expect(code).toBe(0);
    const result = JSON.parse(stdout);
...
describe("invalid bash regex handling", () => {
  ...
  function createTempSkillWithRegex(bashPatterns: string[]): {
    hookPath: string;
    root: string;
  } {
    const tempRoot = join(
      tmpdir(),
      `vp-test-regex-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
    const tempHooksDir = join(tempRoot, "hooks");
  ...
  test("emits BASH_REGEX_INVALID for broken regex, still exits 0 with valid JSON, and valid patterns still match", async () => {
    const { hookPath, root } = createTempSkillWithRegex([
      "(unclosed-group",
      "\\bvalid-command\\b",
    ]);
...
describe("invalid glob pattern handling", () => {
  ...
  function createTempSkillWithBadGlob(): { hookPath: string; root: string } {
    const tempRoot = join(
      tmpdir(),
      `vp-test-glob-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
    const tempHooksDir = join(tempRoot, "hooks");

    // Copy hook + frontmatter parser
    // Patched patterns.mjs: throws on "__THROW__" sentinel, delegates otherwise
    const realPatterns = readFileSync(
      join(ROOT, "hooks", "patterns.mjs"),
      "utf-8",
    );
...
describe("coverage matrix — file paths", () => {
  ...
  test("apps/web/src/components/ui/dialog.tsx → shadcn (monorepo)", async () => {
    const skills = await matchFile(
      "/project/apps/web/src/components/ui/dialog.tsx",
    );
    expect(skills).toContain("shadcn");
  });
...
describe("specialist wins over generalist in overlap", () => {
  ...
  test("monorepo apps/web/app/api/chat/route.ts: ai-sdk before nextjs", async () => {
    const skills = await matchFileOrdered(
      "/project/apps/web/app/api/chat/route.ts",
    );
...
describe("vercel-firewall priority ranks above vercel-cli", () => {
  ...
  test(".vercel/firewall/config.json: vercel-firewall appears before vercel-cli", async () => {
    const skills = await matchFileOrdered(
      "/project/.vercel/firewall/config.json",
    );
...
describe("hooks.json PreToolUse config", () => {
  test("does not auto-register the Read|Edit|Write|Bash skill injection hook", () => {
    ...
    const skillInjectionEntry = preToolHooks.find(
      (entry: any) =>
...
describe("validateSkillMap", () => {
  ...
  test("returns error when skills key is missing", () => {
    ...
    expect(result.errors).toContain(
      "skill-map is missing required 'skills' key",
    );
  });
  ...
  test("warns and defaults NaN priority to 5", () => {
    const result = validateSkillMap({
      skills: {
        "test-skill": { priority: NaN, pathPatterns: [], bashPatterns: [] },
      },
    });
    expect(result.ok).toBe(true);
    expect(result.normalizedSkillMap.skills["test-skill"].priority).toBe(5);
    expect(
      result.warnings.some(
        (w: string) =>
          w.includes("priority") && w.includes("not a valid number"),
      ),
    ).toBe(true);
  });
  ...
  test("warns and defaults non-number priority to 5", () => {
    const result = validateSkillMap({
      skills: {
        "test-skill": { priority: "high", pathPatterns: [], bashPatterns: [] },
      },
    });
    expect(result.ok).toBe(true);
    expect(result.normalizedSkillMap.skills["test-skill"].priority).toBe(5);
  ...
  test("warns on non-array pathPatterns and defaults to []", () => {
    const result = validateSkillMap({
      skills: {
        "test-skill": { priority: 1, pathPatterns: "*.ts", bashPatterns: [] },
      },
    });
    expect(result.ok).toBe(true);
    expect(result.normalizedSkillMap.skills["test-skill"].pathPatterns).toEqual(
    ...
    expect(
      result.warnings.some(
        (w: string) => w.includes("pathPatterns") && w.includes("not an array"),
      ),
    ).toBe(true);
  });
  ...
  test("removes non-string entries from pathPatterns with warning", () => {
    ...
    expect(
      result.warnings.some(
        (w: string) =>
          w.includes("pathPatterns[1]") && w.includes("not a string"),
      ),
    ).toBe(true);
  });
  ...
  test("removes non-string entries from bashPatterns with warning", () => {
    ...
    expect(
      result.warnings.some((w: string) => w.includes("bashPatterns[1]")),
    ).toBe(true);
  });
  ...
  test("warns on unknown keys", () => {
    ...
    expect(
      result.warnings.some((w: string) =>
        w.includes('unknown key "description"'),
      ),
    ).toBe(true);
    expect(
  ...
  test("validates the frontmatter-built skill map successfully", async () => {
    ...
    expect(
      Object.keys(result.normalizedSkillMap.skills).length,
    ).toBeGreaterThan(0);
  });
  ...
  test("validates buildSkillMap output with coerced bare-string pathPatterns", async () => {
    ...
    expect(
      result.normalizedSkillMap.skills["coerce-skill"].bashPatterns,
    ).toEqual(["\\bnpm\\b"]);
...
describe("deterministic ordering", () => {
  test("tie-priority skills produce identical order across multiple runs", async () => {
    ...
    const entries = Object.entries(customMap.skills).map(
      ([skill, config]: [string, any]) => ({
        skill,
    ...
    for (let i = 0; i < 10; i++) {
      ...
      shuffled.sort(
        (a, b) => b.priority - a.priority || a.skill.localeCompare(b.skill),
      );
...
describe("redactCommand", () => {
  ...
  test("masks TOKEN= values", () => {
    expect(
      redactCommand("curl -H TOKEN=abc123secret https://api.example.com"),
  ...
  test("masks KEY= values preserving full key name", () => {
    expect(redactCommand("VERCEL_API_KEY=sk_live_xyz deploy")).toContain(
      "VERCEL_API_KEY=[REDACTED]",
    );
    expect(redactCommand("VERCEL_API_KEY=sk_live_xyz deploy")).not.toContain(
      "sk_live_xyz",
    );
  });

  test("masks SECRET= values preserving full key name", () => {
    expect(redactCommand("MY_SECRET=hunter2 run")).toContain(
      "MY_SECRET=[REDACTED]",
    );
  ...
  test("masks --token flag values", () => {
    expect(redactCommand("vercel --token tk_abcdef deploy")).toContain(
      "--token [REDACTED]",
    );
    expect(redactCommand("vercel --token tk_abcdef deploy")).not.toContain(
      "tk_abcdef",
    );
  });

  test("masks --password flag values", () => {
    expect(redactCommand("mysql --password s3cret -u root")).toContain(
      "--password [REDACTED]",
    );
    expect(redactCommand("mysql --password s3cret -u root")).not.toContain(
      "s3cret",
    );
  });

  test("masks --api-key flag values", () => {
    expect(redactCommand("cli --api-key my-key-123")).toContain(
      "--api-key [REDACTED]",
    );
    expect(redactCommand("cli --api-key my-key-123")).not.toContain(
      "my-key-123",
    );
  });
  ...
  test("preserves full env var key name with prefix (regression)", () => {
    ...
    expect(redactCommand("VERCEL_TOKEN=abc123")).toBe(
    ...
    expect(redactCommand("MY_SECRET_VALUE=hunter2")).toBe(
      "MY_SECRET_VALUE=[REDACTED]",
    );
    expect(redactCommand("CREDENTIAL_STORE=val")).toBe(
  ...
  test("masks Bearer tokens", () => {
    expect(
      redactCommand("curl -H 'Authorization: Bearer eyJhbGciOiJIUzI1NiJ9.abc'"),
  ...
  test("masks 'token xxx' authorization style", () => {
    expect(
      redactCommand("gh api -H 'Authorization: token ghp_abc123XYZ456'"),
    ).toContain("token [REDACTED]");
    expect(
      redactCommand("gh api -H 'Authorization: token ghp_abc123XYZ456'"),
    ).not.toContain("ghp_abc123XYZ456");
  });
  ...
  test("masks JSON password and api_key values", () => {
    expect(redactCommand('{"password": "hunter2"}')).toContain(
      '"password": "[REDACTED]"',
    );
    expect(redactCommand('{"api_key": "ak_xyz"}')).toContain(
      '"api_key": "[REDACTED]"',
    );
    expect(redactCommand('{"apiKey": "ak_xyz"}')).toContain(
      '"apiKey": "[REDACTED]"',
    );
  });
  ...
  test("masks Cookie headers", () => {
    const cmd =
      "curl -H 'Cookie: session=abc123; auth_tok=xyz789' https://example.com";
    const result = redactCommand(cmd);
  ...
  test("masks --secret and --auth flags", () => {
    expect(redactCommand("tool --secret mysecretval")).toContain(
      "--secret [REDACTED]",
    );
    expect(redactCommand("tool --auth bearer_tok")).toContain(
      "--auth [REDACTED]",
    );
  });

  test("masks PASSWORD= env vars", () => {
    expect(redactCommand("DB_PASSWORD=hunter2 npm start")).toBe(
      "DB_PASSWORD=[REDACTED] npm start",
    );
  });
});
...
describe("importPatternToRegex", () => {
  ...
  test("matches sub-path import @ai-sdk/gateway/foo", () => {
    const re = importPatternToRegex("@ai-sdk/gateway");
    expect(re.test(`import { thing } from '@ai-sdk/gateway/providers'`)).toBe(
      true,
    );
  });
...
describe("matchImportWithReason", () => {
  ...
  test("returns match with matchType 'import' for matching content", () => {
    ...
    const compiled = patterns.map((p: string) => ({
      pattern: p,
      regex: importPatternToRegex(p),
    }));
...
describe("decision logging — reason codes", () => {
  ...
  test("reason=no_matches when tool is supported but nothing matches", async () => {
    const { stderr } = await runHookEnv(
      {
        tool_name: "Read",
        tool_input: { file_path: "/totally/unknown/file.xyz" },
      },
      { VERCEL_PLUGIN_HOOK_DEBUG: "1" },
    );
    const complete = parseComplete(stderr);
  ...
  describe("hook output schema compliance", () => {
    test("matched skill output has no unknown keys in hookSpecificOutput", async () => {
      ...
      const unknownKeys = keys.filter(
        (k) => !ALLOWED_HOOK_SPECIFIC_KEYS.has(k),
      );
