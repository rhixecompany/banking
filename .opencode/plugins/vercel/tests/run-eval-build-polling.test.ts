...
describe("isExpectedRunCommandTimeout", () => {
  test("matches the expected sandbox API timeout error", () => {
    expect(
      isExpectedRunCommandTimeout(
        new Error("runCommand timed out after 300000ms"),
      ),
    ).toBe(true);
    expect(
      isExpectedRunCommandTimeout(
        new Error("deadline exceeded while waiting for response"),
      ),
    ).toBe(true);
  });

  test("does not hide unrelated build launch failures", () => {
    expect(isExpectedRunCommandTimeout(new Error("permission denied"))).toBe(
      false,
    );
  });
});
...
describe("renderTimingMarkdownTable", () => {
  test("renders per-scenario phase durations and marks skipped phases", () => {
    ...
    expect(markdown).toContain(
      "| timed-scenario | 1.0s | 2.0s | 3.0s | 15.0s | 5.0s | skip | 4.0s |",
    );
  });
});
...
describe("summarizeToolCalls", () => {
  ...
  test("returns a stable fallback when no hook calls were found", () => {
    expect(summarizeToolCalls("(cmd failed)")).toBe(
      "No executePreToolHooks entries found.",
    );
  });
});
