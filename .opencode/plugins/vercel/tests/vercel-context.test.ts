...
describe("managed vercel context chunks", () => {
  test("extracts a small nextjs chunk from vercel.md", () => {
    const chunk = getManagedContextChunkForSkill("nextjs", {
      pluginRoot: ROOT,
    });
  ...
  test("returns null for unmapped skills", () => {
    expect(
      getManagedContextChunkForSkill("shadcn", { pluginRoot: ROOT }),
    ).toBeNull();
  });
...
describe("on-demand context injection", () => {
  ...
  test("prompt hook appends ai chunk once and dedups it across later prompts", async () => {
    ...
    try {
      ...
      const firstCtx = firstParsed.hookSpecificOutput
      ...
      const secondCtx = secondParsed.hookSpecificOutput
        .additionalContext as string;
  ...
  test("prompt audit log records context chunk ids", async () => {
    ...
    try {
      ...
      const promptEntry = entries.find(
        (entry) => entry.event === "prompt-skill-injection",
      );
