...
describe("Verification companion injection", () => {
  ...
  test("skillInjection metadata includes reasons map and verificationId", async () => {
    ...
    expect(injection.reasons["agent-browser-verify"].trigger).toBe(
      "dev-server-start",
    );
    expect(injection.reasons["agent-browser-verify"].reasonCode).toBe(
      "bash-dev-server-pattern",
    );
    expect(injection.reasons["verification"]).toBeDefined();
    expect(injection.reasons["verification"].trigger).toBe(
