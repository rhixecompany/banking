...
describe("parseSkillFrontmatter", () => {
  ...
  test("parses skills/ncc frontmatter with regex chaining intact", () => {
    ...
    expect(result.chainTo[1].pattern).toBe(
  ...
  test("quotes @repo/* in next-forge YAML frontmatter for standard YAML parser compatibility", () => {
    const overlay = readFileSync(
      join(SKILLS_DIR, "next-forge", "overlay.yaml"),
      "utf-8",
    );
...
describe("buildSkillMap", () => {
  ...
  test("backslash sequences preserved in bash patterns", () => {
    ...
    const hasWordBoundary = nextjs.bashPatterns.some((p: string) =>
      p.includes("\\b"),
    );
  ...
  test("coerces bare string pathPatterns to array with warning", () => {
    ...
    expect(
      map.warnings.some(
        (w: string) => w.includes("pathPatterns") && w.includes("coercing"),
      ),
    ).toBe(true);
  ...
  test("coerces bare string bashPatterns to array with warning", () => {
    ...
    expect(
      map.warnings.some(
        (w: string) => w.includes("bashPatterns") && w.includes("coercing"),
      ),
    ).toBe(true);
  ...
  test("summary field is propagated to skill map entry", () => {
    ...
    expect(map.skills["summary-skill"].summary).toBe(
      "This skill provides X guidance.",
    );
...
describe("buildSkillMap — BOM and metadata edge cases", () => {
  function buildWithContent(dirName: string, content: string) {
    const tmp = join(
      tmpdir(),
      `skill-edge-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
...
describe("buildSkillMap — deprecated key compat", () => {
  function buildWithFrontmatter(metadata: string) {
    const tmp = join(
      tmpdir(),
      `skill-compat-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
  ...
  test("filePattern is normalized to pathPatterns with DEPRECATED_FIELD warning", () => {
    const map = buildWithFrontmatter(
      "  filePattern:\n    - 'src/**'\n  bashPatterns: []",
    );
    ...
    expect(
      map.warnings.some(
        (w: string) => w.includes("filePattern") && w.includes("deprecated"),
      ),
    ).toBe(true);
    const detail = map.warningDetails.find(
      (d: any) => d.code === "DEPRECATED_FIELD" && d.field === "filePattern",
    );
  ...
  test("bashPattern is normalized to bashPatterns with DEPRECATED_FIELD warning", () => {
    ...
    expect(
      map.warnings.some(
        (w: string) => w.includes("bashPattern") && w.includes("deprecated"),
      ),
    ).toBe(true);
    const detail = map.warningDetails.find(
      (d: any) => d.code === "DEPRECATED_FIELD" && d.field === "bashPattern",
    );
    expect(detail).toBeDefined();
  });
  ...
  test("canonical pathPatterns takes precedence over deprecated filePattern", () => {
    ...
    expect(map.warnings.some((w: string) => w.includes("filePattern"))).toBe(
      false,
    );
  });
...
describe("buildSkillMap — malformed array entries", () => {
  function buildWithFrontmatter(metadata: string) {
    const tmp = join(
      tmpdir(),
      `skill-malformed-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
  ...
  test("pathPatterns: [42] filters out non-string with warning", () => {
    ...
    expect(
      map.warnings.some(
        (w: string) =>
          w.includes("pathPatterns[0]") && w.includes("not a string"),
      ),
    ).toBe(true);
  });
  ...
  test("pathPatterns: [''] filters out empty string with warning", () => {
    ...
    expect(
      map.warnings.some(
        (w: string) => w.includes("pathPatterns[0]") && w.includes("empty"),
      ),
    ).toBe(true);
  });
  ...
  test("bashPatterns: [42] filters out non-string with warning", () => {
    ...
    expect(
      map.warnings.some(
        (w: string) =>
          w.includes("bashPatterns[0]") && w.includes("not a string"),
      ),
    ).toBe(true);
  });
  ...
  test("bashPatterns: [''] filters out empty string with warning", () => {
    ...
    expect(
      map.warnings.some(
        (w: string) => w.includes("bashPatterns[0]") && w.includes("empty"),
      ),
    ).toBe(true);
  });
});
...
describe("buildSkillMap — warningDetails structured diagnostics", () => {
  function buildWithFrontmatter(metadata: string) {
    const tmp = join(
      tmpdir(),
      `skill-wd-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
    const skillDir = join(tmp, "test-skill");
    mkdirSync(skillDir, { recursive: true });
    writeFileSync(
  ...
  test("coercing string pathPatterns produces structured detail with COERCE_STRING_TO_ARRAY", () => {
    ...
    const detail = map.warningDetails.find(
      (d: any) =>
        d.code === "COERCE_STRING_TO_ARRAY" && d.field === "pathPatterns",
    );
    expect(detail).toBeDefined();
  ...
  test("non-array pathPatterns produces INVALID_TYPE detail", () => {
    ...
    const detail = map.warningDetails.find(
      (d: any) => d.code === "INVALID_TYPE" && d.field === "pathPatterns",
    );
    expect(detail).toBeDefined();
  ...
  test("non-string entry in pathPatterns produces ENTRY_NOT_STRING detail", () => {
    ...
    const detail = map.warningDetails.find(
      (d: any) =>
        d.code === "ENTRY_NOT_STRING" && d.field === "pathPatterns[0]",
    );
    expect(detail).toBeDefined();
    expect(detail.skill).toBe("test-skill");
  });
  ...
  test("empty string in bashPatterns produces ENTRY_EMPTY detail", () => {
    ...
    const detail = map.warningDetails.find(
      (d: any) => d.code === "ENTRY_EMPTY" && d.field === "bashPatterns[0]",
    );
    expect(detail).toBeDefined();
  });
...
describe("validateSkillMap — structured errorDetails and warningDetails", () => {
  ...
  test("unknown key produces UNKNOWN_KEY warningDetail", () => {
    const result = validateSkillMap({
      skills: {
        s1: { priority: 5, pathPatterns: [], bashPatterns: [], extraKey: true },
      },
    });
    expect(result.ok).toBe(true);
    const detail = result.warningDetails.find(
      (d: any) => d.code === "UNKNOWN_KEY" && d.field === "extraKey",
    );
    expect(detail).toBeDefined();
  ...
  test("non-string pathPatterns entry produces ENTRY_NOT_STRING warningDetail", () => {
    const result = validateSkillMap({
      skills: {
        s1: { priority: 5, pathPatterns: [42, "valid/**"], bashPatterns: [] },
      },
    });
    expect(result.ok).toBe(true);
    const detail = result.warningDetails.find(
  ...
  test("warningDetails length matches warnings length on valid input", () => {
    const result = validateSkillMap({
      skills: {
        s1: { priority: "bad", pathPatterns: [42], bashPatterns: ["", "ok"] },
      },
    });
    expect(result.ok).toBe(true);
  ...
  test("non-string summary defaults to empty string", () => {
    const result = validateSkillMap({
      skills: {
        s1: { priority: 5, summary: 42, pathPatterns: [], bashPatterns: [] },
      },
    });
    expect(result.ok).toBe(true);
    expect(result.normalizedSkillMap.skills["s1"].summary).toBe("");
  ...
  test("validate field is preserved through validateSkillMap", () => {
    const rules = [
      { pattern: "import.*openai", message: "Use gateway", severity: "error" },
    ];
    const result = validateSkillMap({
      skills: {
        s1: {
    ...
    expect(result.normalizedSkillMap.skills["s1"].validate[0].pattern).toBe(
      "import.*openai",
    );
  });
  ...
  test("validate is not flagged as unknown key", () => {
    const result = validateSkillMap({
      skills: {
        s1: { priority: 5, pathPatterns: [], bashPatterns: [], validate: [] },
      },
    });
    expect(result.ok).toBe(true);
    expect(result.warnings.length).toBe(0);
    ...
    const result = validateSkillMap({
      skills: {
        s1: {
          ...
          bashPatterns: [],
          validate: [
            { pattern: "ok", message: "msg", severity: "error" },
            { bad: true },
          ],
        },
      },
  ...
  test("config not object produces CONFIG_NOT_OBJECT errorDetail", () => {
    ...
    expect(result.ok).toBe(false);
    const detail = result.errorDetails.find(
      (d: any) => d.code === "CONFIG_NOT_OBJECT",
    );
    expect(detail).toBeDefined();
    expect(detail.skill).toBe("s1");
  });
...
describe("validateSkillMap — malformed array entries", () => {
  test("pathPatterns with non-string entry is filtered with warning", () => {
    ...
    expect(result.ok).toBe(true);
    expect(result.normalizedSkillMap.skills["s1"].pathPatterns).toEqual([
      "valid/**",
    ]);
    expect(
  ...
  test("pathPatterns with empty string is filtered with warning", () => {
    const result = validateSkillMap({
      skills: {
        s1: { priority: 5, pathPatterns: ["", "valid/**"], bashPatterns: [] },
      },
    });
    expect(result.ok).toBe(true);
    expect(result.normalizedSkillMap.skills["s1"].pathPatterns).toEqual([
    ...
    const result = validateSkillMap({
      skills: {
        s1: {
          priority: 5,
          pathPatterns: [],
          bashPatterns: [null, "\\bvalid\\b"],
        },
      },
    });
    expect(result.ok).toBe(true);
    expect(result.normalizedSkillMap.skills["s1"].bashPatterns).toEqual([
      "\\bvalid\\b",
    ]);
    expect(
      result.warnings.some(
...
describe("validateSkillMap — promptSignals", () => {
  ...
  test("promptSignals is preserved through validation normalization", () => {
    ...
    expect(
      result.normalizedSkillMap.skills["s1"].promptSignals!.phrases,
    ).toEqual(["ai elements", "streaming markdown"]);
    expect(
      result.normalizedSkillMap.skills["s1"].promptSignals!.noneOf,
    ).toEqual(["readme", "markdown file"]);
  });
  ...
  test("missing promptSignals is omitted (not defaulted) in normalized output", () => {
    ...
    expect(result.ok).toBe(true);
    expect(
      result.normalizedSkillMap.skills["s1"].promptSignals,
    ).toBeUndefined();
  });
  ...
  test("real skills directory produces no warnings (promptSignals included)", () => {
    ...
    const withSignals = Object.entries(result.normalizedSkillMap.skills).filter(
      ([, cfg]: [string, any]) => cfg.promptSignals,
    );
...
describe("promptSignals malformed warnings via buildSkillMap", () => {
  function buildWithSignals(promptSignalsYaml: string) {
    const tmp = join(
      tmpdir(),
      `skill-signals-warn-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
    );
    const skillDir = join(tmp, "test-skill");
    mkdirSync(skillDir, { recursive: true });
    writeFileSync(
...
describe("parseSkillFrontmatter — duplicate key detection", () => {
  ...
  test("throws on duplicate nested key inside metadata", () => {
    ...
    expect(() => parseSkillFrontmatter(yaml)).toThrow(
  ...
  test("throws on duplicate chainTo key (the original bug)", () => {
    ...
    expect(() => parseSkillFrontmatter(yaml)).toThrow(
      /duplicate key "chainTo"/,
    );
  });

  test("non-duplicate keys with similar names parse fine", () => {
    const yaml = ["name: my-skill", "description: test", "summary: brief"].join(
      "\n",
    );
