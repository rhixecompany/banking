...
const { synthesizeChainToFromValidate } =
  await import("../scripts/build-manifest.ts");
...
describe("build-manifest.ts", () => {
  ...
  test("each manifest skill has required fields", () => {
    const manifest = readManifest();
    for (const [slug, config] of Object.entries(manifest.skills) as [
      string,
      any,
    ][]) {
...
describe("manifest-backed hook loading", () => {
  ...
  test("hook produces same matches with and without manifest", async () => {
    ...
    try {
      ...
      const withSkills = withMatch
        ? (JSON.parse(withMatch[1]).injectedSkills ?? [])
        : [];
      const withoutCtx =
        withoutOutput.hookSpecificOutput?.additionalContext || "";
      const withoutMatch = withoutCtx.match(
        /<!-- skillInjection: (\{.*?\}) -->/,
      );
      const withoutSkills = withoutMatch
        ? (JSON.parse(withoutMatch[1]).injectedSkills ?? [])
        : [];
...
`,
  ...
  test("loadSkills falls back on manifest without skills key", async () => {
    writeFileSync(
      join(GEN, "skill-manifest.json"),
      JSON.stringify({ generatedAt: "x" }),
    );
...
describe("synthesizeChainToFromValidate", () => {
  ...
  test("synthesizes chainTo from upgradeToSkill with severity recommended", () => {
    ...
    expect(skills["skill-a"].chainTo[0].message).toContain(
      "loading skill-b guidance",
    );
  });
...
describe("manifest includes synthesized chainTo entries", () => {
  test("build succeeds and all chainTo entries have valid structure", async () => {
    ...
    for (const [_slug, config] of Object.entries(manifest.skills) as [
      string,
      any,
    ][]) {
      if (config.chainTo) {
