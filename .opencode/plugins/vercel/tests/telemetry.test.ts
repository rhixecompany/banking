...
async function runTelemetryProbe(options: { telemetryEnv?: string }): Promise<{
  dauEnabled: boolean;
  calls: number;
  stampPath: string;
  dauPayloads: unknown[];
...
describe("telemetry controls", () => {
  ...
  test("default telemetry is DAU-only", async () => {
    ...
    expect(result.stampPath).toBe(
      join(tempHome, ".config", "vercel-plugin", "dau-stamp"),
    );
  ...
  test("compiled hooks do not emit prompt, tool, or skill-injection telemetry keys", () => {
    const pretoolHook = readFileSync(
      join(ROOT, "hooks", "pretooluse-skill-inject.mjs"),
      "utf-8",
    );
    const promptSkillInjectHook = readFileSync(
      join(ROOT, "hooks", "user-prompt-submit-skill-inject.mjs"),
      "utf-8",
    );
  ...
  test("session-start profiler source only references the DAU ping telemetry key", () => {
    const profilerHook = readFileSync(
      join(ROOT, "hooks", "session-start-profiler.mjs"),
      "utf-8",
    );
