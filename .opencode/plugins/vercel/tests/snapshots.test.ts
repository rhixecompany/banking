...
const PAYLOADS_PATH = join(
  ROOT,
  "tests",
  "fixtures",
  "consolidated-payloads.json",
);
...
describe("golden payload snapshots", () => {
  for (const fixture of payloads.fixtures) {
    test(`golden: ${fixture.name}`, async () => {
      const {
        code,
        skillInjection: actual,
        additionalContext,
      } = await runHook(fixture.input);
      expect(code).toBe(0);
      ...
      const summaryOnlyLen = Array.isArray(actual!.summaryOnly)
        ? (actual!.summaryOnly as string[]).length
        : 0;
      expect(
