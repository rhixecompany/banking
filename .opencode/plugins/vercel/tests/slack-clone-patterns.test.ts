...
async function runHook(input: {
  ...
  proc.stdin.write(
    JSON.stringify({ ...input, session_id: "slack-clone-patterns" }),
  );
  proc.stdin.end();
...
const EXPECTED_SLACK_ROUTE_SKILLS = [
  "chat-sdk",
  "vercel-functions",
  "nextjs",
] as const;
...
describe("slack clone patterns", () => {
  test("slack-clone app/api/slack/route.ts injects chat-sdk, vercel-functions, nextjs", async () => {
    expect(
      await matchFile("/Users/me/slack-clone/app/api/slack/route.ts"),
    ).toEqual(EXPECTED_SLACK_ROUTE_SKILLS);
  });

  test("slack-clone src/app/api/slack/route.ts injects chat-sdk, vercel-functions, nextjs", async () => {
    expect(
      await matchFile("/Users/me/slack-clone/src/app/api/slack/route.ts"),
    ).toEqual(EXPECTED_SLACK_ROUTE_SKILLS);
  });

  test("slack-clone app/api/webhooks/slack/route.ts injects chat-sdk, vercel-functions, nextjs", async () => {
    expect(
      await matchFile("/Users/me/slack-clone/app/api/webhooks/slack/route.ts"),
    ).toEqual(EXPECTED_SLACK_ROUTE_SKILLS);
  });

  test("slack-clone lib/bot/slack.ts injects chat-sdk", async () => {
    expect(await matchFile("/Users/me/slack-clone/lib/bot/slack.ts")).toEqual([
      "chat-sdk",
    ]);
  });
  ...
  test("slack-clone components/chat/message-list.tsx injects json-render", async () => {
    expect(
      await matchFile("/Users/me/slack-clone/components/chat/message-list.tsx"),
    ).toEqual(["json-render"]);
  });
});
