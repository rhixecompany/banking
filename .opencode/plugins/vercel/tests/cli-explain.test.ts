...
describe("budget-aware injection", () => {
  test("json includes injectionMode and budget fields", async () => {
    ...
    for (const m of result.matches) {
      ...
      expect(["full", "summary", "droppedByCap", "droppedByBudget"]).toContain(
        m.injectionMode,
      );
    }
  });
  ...
  test("tiny budget forces budget drops", async () => {
    ...
    const fullCount = result.matches.filter(
      (m: any) => m.injectionMode === "full",
    ).length;
...
describe("cap behavior", () => {
  ...
  test("large budget allows MAX_SKILLS cap to apply", async () => {
    ...
    if (result.matches.length > 3) {
      ...
      const cappedByHardCeiling = result.matches.filter(
        (m: any) => m.injectionMode === "droppedByCap",
      );
