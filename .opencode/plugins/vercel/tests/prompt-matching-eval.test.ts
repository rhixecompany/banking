...
const corpusPath = resolve(import.meta.dir, "fixtures/prompt-eval-corpus.json");
...
describe("prompt matching eval harness", () => {
  ...
  test("evaluate all prompts and report precision/recall", () => {
    ...
    for (const entry of corpus.corpus) {
      ...
      const noFP =
        entry.expectedSkills.length === 0 ? selected.length === 0 : true;
  ...
  test("lexical coverage delta: compare exact-only vs lexical mode", () => {
    ...
    expect(lexStats.overall.recall).toBeGreaterThanOrEqual(
      exactStats.overall.recall,
    );
