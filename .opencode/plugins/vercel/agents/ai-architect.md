...
title: Type-Safe useChat with Agents description: Build end-to-end type-safe agents by inferring UIMessage types from your agent definition.
...
export type WeatherToolInvocation = UIToolInvocation<
  typeof weatherTool
>;
```
...
export function WeatherToolComponent({
  ...
  return (
    <div>Loading weather for {invocation.input?.location}...</div>
  );
}
```
    ...
    <div>
      {message.parts.map((part, i) => {
        switch (part.type) {
          ...
          case "tool-calculator":
            return (
              <CalculatorToolComponent key={i} invocation={part} />
            );
          default:
            return null;
        }
...
title: Common Errors description: Reference for common AI SDK errors and how to resolve them.
