...
export async function analyzeSessionJsonl(
  jsonlPath: string,
...
function extractTraceInjectionsFromUnknown(
  ...
  while (queue.length > 0) {
    ...
    const maybeInjection = makeTraceInjectionFromRecord(
      record,
      fallbackToolName,
    );
...
async function parseHookProgressInjectionsFromSessionJsonl(
  jsonlPath: string,
): Promise<TraceInjection[]> {
...
async function discoverBenchmarkAgentSlugEntries(
  baseDir: string,
): Promise<Array<{ slug: string; cwd: string }>> {
...
async function main() {
  ...
  if (manifest) {
    ...
    if (slugDirs.length > 0) {
      ...
      const discoveredFromSessions =
        await discoverBenchmarkAgentSlugEntries(BASE_DIR);
      if (discoveredFromSessions.length > 0) {
        ...
        console.log(
          `No benchmark project sessions found in ${CLAUDE_PROJECTS_DIR}`,
        );
  ...
  for (const { slug, cwd: projectDir } of slugEntries.sort((a, b) =>
    ...
    const meta = await safeReadJson<RunMeta>(
    ...
    if (jsonls.length > 0) {
      ...
      if (!auditLog.found && traceInjections.length === 0) {
        const hookProgressInjections =
          await parseHookProgressInjectionsFromSessionJsonl(jsonls[0]);
  ...
  const aggregate = {
    ...
    avgMatchScore:
      scorecards.length > 0
        ? scorecards.reduce((sum, s) => sum + s.matchScore, 0) /
          scorecards.length
        : 0,
    ...
    allMissingSkills: [
      ...new Set(scorecards.flatMap((s) => s.missingSkills)),
    ].sort(),
    allUnexpectedSkills: [
      ...new Set(scorecards.flatMap((s) => s.unexpectedSkills)),
    ].sort(),
  ...
  console.log(
    `Total tokens: ${aggregate.totalTokensIn} in / ${aggregate.totalTokensOut} out`,
  );
