 ...
 */
function parseHeadingSpec(spec: string): {
  level: number | null;
  text: string;
} {
...
function extractDirectSection(markdown: string, headingSpec: string): string {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (
      lineText === specText &&
      (specLevel === null || lineLevel === specLevel)
    ) {
      startLine = i;
...
function resolveIncludes(
  ...
  const output = template.replace(
    INCLUDE_RE,
    (fullMatch, skillName: string, target: string, offset: number) => {
...
if (import.meta.main) {
  ...
  for (const dir of tmplDirs) {
    ...
    for (const f of readdirSync(dir)) {
      ...
      if (
        filters.length === 0 ||
        filters.some((flt) => fullPath.includes(flt))
      ) {
  ...
  if (audit) {
    ...
    if (jsonMode) {
      ...
      for (const entry of auditResults) {
        ...
        const status = entry.migrationCandidate
          ? "⚠ MIGRATION CANDIDATE"
          : "✓ ok";
        console.log(
