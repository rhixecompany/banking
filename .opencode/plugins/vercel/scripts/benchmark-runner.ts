...
async function main() {
  ...
  console.log(
    `Projects: ${projects.length} (${flags.quick ? "quick" : "full"})`,
  );
  ...
  for (const project of projects) {
    ...
    try {
      ...
      console.log(
        `  Running claude --print (timeout ${TIMEOUT_MS / 1000}s)...`,
      );
      ...
      // Step 3: Save outputs
      await writeFile(
        join(projectResultsDir, "claude-output.txt"),
        result.claudeOutput,
      );
  ...
  console.log(`\n=== Summary ===`);
  console.log(
    `${"Slug".padEnd(28)} ${"Status".padEnd(10)} ${"Duration".padEnd(10)} Expected Skills`,
  );
