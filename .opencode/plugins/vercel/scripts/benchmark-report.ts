 ...
 */
export const SKILL_PATTERN_HINTS: Record<
  string,
  { glob: string; tool: string }[]
> = {
  auth: [
  ...
  "routing-middleware": [{ glob: "middleware.{ts,js}", tool: "Read" }],
  cms: [
    { glob: "sanity.config.*", tool: "Read" },
    { glob: "content/**", tool: "Read" },
  ],
  observability: [{ glob: "instrumentation.{ts,js}", tool: "Read" }],
  "deployments-cicd": [
...
export function buildSuggestedPatterns(
  missingBySkill: Map<string, string[]>,
): SuggestedPattern[] {
...
function verdict(
  ...
  runEntry: RunnerSummaryEntry | null,
): string {
...
async function main() {
  ...
  const manifest = await safeReadJson<BenchmarkRunManifest>(
    join(RESULTS_DIR, "run-manifest.json"),
  );
  const analysisSummary = await safeReadJson<AnalysisSummary>(
    join(RESULTS_DIR, "analysis-summary.json"),
  );
  const runId = manifest?.runId ?? analysisSummary?.runId ?? null;
  const runnerSummary = await safeReadJson<RunnerSummaryEntry[]>(
    join(RESULTS_DIR, "runner-summary.json"),
  );
  const verifySummary = await safeReadJson<VerifyResult[]>(
    join(RESULTS_DIR, "verify-summary.json"),
  );
  ...
  for (const slug of slugDirs.sort()) {
    const sc = await safeReadJson<Scorecard>(
      join(RESULTS_DIR, slug, "scorecard.json"),
    );
    if (sc) scorecards.push(sc);

    // Also load verify.json per-project if not in summary
    if (!verifyMap.has(slug)) {
      const v = await safeReadJson<VerifyResult>(
        join(RESULTS_DIR, slug, "verify.json"),
      );
  ...
  lines.push(``);
  lines.push(
    `| Slug | Expected Skills | Actual Skills | Skill Match | Dev Server | Tokens (in/out) | Tool Calls | Verdict |`,
  );
  lines.push(
    `|------|----------------|---------------|-------------|------------|-----------------|------------|---------|`,
  );
  ...
  const avgMatchScore =
    scorecards.reduce((s, c) => s + c.matchScore, 0) / scorecards.length;
  const passCount = scorecards.filter((sc) => {
    const v = verifyMap.get(sc.slug);
    return sc.matchScore === 1 && v?.devServer;
  }).length;
  const devServerPassCount = [...verifyMap.values()].filter(
    (v) => v.devServer,
  ).length;
  const skillFullMatchCount = scorecards.filter(
    (sc) => sc.matchScore === 1,
  ).length;
  ...
  lines.push(`|--------|-------|`);
  lines.push(
    `| Overall pass rate (skill match + dev server) | ${passCount}/${scorecards.length} (${pct(passCount / scorecards.length)}) |`,
  );
  lines.push(
    `| Skill injection full match | ${skillFullMatchCount}/${scorecards.length} (${pct(skillFullMatchCount / scorecards.length)}) |`,
  );
  lines.push(
    `| Dev server pass rate | ${devServerPassCount}/${scorecards.length} (${pct(devServerPassCount / scorecards.length)}) |`,
  );
  lines.push(`| Average skill match score | ${pct(avgMatchScore)} |`);
  lines.push(
    `| Total tokens (in / out) | ${fmtTokens(totalTokensIn)} / ${fmtTokens(totalTokensOut)} |`,
  );
  lines.push(
    `| Average tokens per project | ${fmtTokens(totalTokensIn / scorecards.length)} / ${fmtTokens(totalTokensOut / scorecards.length)} |`,
  );
  lines.push(`| Total tool calls | ${totalToolCalls} |`);
  lines.push(
    `| Average tool calls per project | ${(totalToolCalls / scorecards.length).toFixed(0)} |`,
  );
  lines.push(``);
  ...
  if (missingBySkill.size === 0 && unexpectedBySkill.size === 0) {
    lines.push(
      `No coverage gaps detected — all expected skills were injected.`,
    );
  } else {
    if (missingBySkill.size > 0) {
      ...
      lines.push(`|-------|-------------------|-----------|`);
      for (const [skill, slugs] of [...missingBySkill.entries()].sort(
        (a, b) => b[1].length - a[1].length,
      )) {
        lines.push(
          `| ${skill} | ${slugs.join(", ")} | ${slugs.length}/${scorecards.length} |`,
        );
      }
      lines.push(``);
    }
    ...
    if (unexpectedBySkill.size > 0) {
      ...
      lines.push(`|-------|---------------------|-----------|`);
      for (const [skill, slugs] of [...unexpectedBySkill.entries()].sort(
        (a, b) => b[1].length - a[1].length,
      )) {
        lines.push(
          `| ${skill} | ${slugs.join(", ")} | ${slugs.length}/${scorecards.length} |`,
  ...
  const projectsWithBuildErrors = [...verifyMap.values()].filter(
    (v) => v.buildErrors.length > 0,
  );
  ...
  // Token efficiency
  const highTokenProjects = scorecards.filter(
    (sc) => sc.tokens.input > 500_000,
  );
  ...
  if (recommendations.length === 0) {
    lines.push(
      `No issues detected — all skills matched and dev servers passed.`,
    );
  } else {
  ...
  lines.push(``);
  lines.push(
    `Copy the prompt below into a Claude Code session to begin the next improvement cycle:`,
  );
  lines.push(``);
  lines.push("```");
  ...
  nextPromptParts.push(
    `Overall: ${passCount}/${scorecards.length} full pass, ${pct(avgMatchScore)} avg skill match, ${devServerPassCount}/${scorecards.length} dev server pass.`,
  );
  nextPromptParts.push(``);
  ...
  if (devServerFailures.length > 0) {
    nextPromptParts.push(
      `Dev server failures: ${devServerFailures.map((v) => v.slug).join(", ")}`,
    );
    nextPromptParts.push(``);
  }
  ...
  if (droppedByCap.size > 0 || droppedByBudget.size > 0) {
    nextPromptParts.push(
      `Dropped skills — cap: [${[...droppedByCap].join(", ")}], budget: [${[...droppedByBudget].join(", ")}]`,
    );
    nextPromptParts.push(``);
  }
  ...
  nextPromptParts.push(``);
  nextPromptParts.push(
    `Implement these fixes in the vercel-plugin repo, then re-run the benchmark suite with: bun run scripts/benchmark-runner.ts && bun run scripts/benchmark-verify.ts && bun run scripts/benchmark-analyze.ts && bun run scripts/benchmark-report.ts`,
  );
  ...
  if (suggestedPatterns.length > 0) {
    ...
    lines.push(``);
    lines.push(
      `Copy-pasteable YAML frontmatter entries for skills that were expected but not injected:`,
    );
    lines.push(``);
