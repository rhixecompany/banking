...
\`\`\`typescript // opencode.jsonc configuration { // Disable MCP tools globally "tools": { "context7*": false, "exa*": false }, // Enable only for specific agent "agent": { "researcher": { "tools": { "context7*": true, "exa*": true } } } } \`\`\`

**Source:** `sst/opencode/packages/opencode/src/util/wildcard.ts:L5-L20`

Wildcard matching implementation:

\`\`\`typescript export function matchWildcard(pattern: string, value: string): boolean { const regex = new RegExp("^" + pattern.replace(/\*/g, ".\*") + "$"); return regex.test(value); } \`\`\`
...
```markdown
OpenCode has per-agent configuration. You can configure tools in opencode.jsonc. Check the docs for more details.
```
