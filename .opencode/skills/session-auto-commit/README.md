...
description: "Automatically commits and pushes changes when a opencode coding agent session ends"
tags: ["automation", "git", "productivity"]
---

# Session Auto-Commit Hook

Automatically commits and pushes changes when a opencode coding agent session ends, ensuring your work is always saved and backed up.

## Overview

This hook runs at the end of each opencode coding agent session and automatically:
...
- **Automatic Backup**: Never lose work from a opencode session
...
1. Copy this hook folder to your repository's `.opencode/skills/` directory:
   ...
   ```bash
   cp -r hooks/session-auto-commit .opencode/skills/
   ```

2. Ensure the script is executable:

   ```bash
   chmod +x .opencode/skills/session-auto-commit/auto-commit.sh
   ```
...
```json
{
  "hooks": {
    "sessionEnd": [
      {
        "type": "command",
        "bash": ".opencode/skills/session-auto-commit/auto-commit.sh",
        "timeoutSec": 30
      }
    ]
  },
...
1. When a opencode coding agent session ends, the hook executes
...
- Works with both opencode coding agent and GitHub opencode CLI
