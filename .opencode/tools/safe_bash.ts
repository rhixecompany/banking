...
const DANGEROUS_PATTERNS = [
  ...
  { pattern: /rm\s+-rf\s+\.\./i, message: "Recursive delete of parent directory" },
  { pattern: /rm\s+-rf\s+\*/i, message: "Recursive delete of all files" },
  
  // Disk wipe commands
  { pattern: /dd\s+.*of=\/dev\/(sd|hd|nvme|vd)/i, message: "Direct disk write operation" },
  ...
  { pattern: /while\s+true\s+do\s+exec/i, message: "Infinite loop fork pattern" },
  
  // System modification
  { pattern: /chmod\s+-R\s+777\s+\//i, message: "Recursive 777 permissions on root" },
  { pattern: /chown\s+-R\s+root/i, message: "Recursive ownership change to root" },
  ...
  { pattern: /curl\s+.*\|\s*sh/i, message: "Pipe to shell (curl-based remote execution)" },
  { pattern: /wget\s+.*\|\s*sh/i, message: "Pipe to shell (wget-based remote execution)" },
 ...
 */
function checkPathTraversal(command: string): { blocked: boolean; reason?: string } {
 ...
 */
function checkAllowlist(command: string): { blocked: boolean; reason?: string } {
...
function sanitizeForLog(command: string): string {
  ...
  sanitized = sanitized.replace(/(password|secret|key|token)=[^\s]+/gi, '$1=***');
  return sanitized;
}
 ...
 */
export default tool({
  description: "Execute safe bash commands with security controls and audit logging",
  args: {
    command: tool.schema.string({
      description: "The bash command to execute",
    }),
    allowlist: tool.schema.boolean({
      description: "Enable allowlist mode (only allow predefined safe commands)",
      required: false,
    }),
