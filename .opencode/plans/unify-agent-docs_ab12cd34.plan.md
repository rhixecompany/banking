...
- Aggressively normalize code snippets in docs and optionally update application source files when explicitly requested by maintainers (this plan will include a source-change step for bcryptjs→bcrypt as requested).
...
4. Normalize SKILL.md headers and aggressively update code snippets in docs and source for the bcryptjs→bcrypt migration (add Zod .describe() where missing, align env usage in snippets to app-config.ts; tag changed snippets with a docs-note comment). When updating source, run validation commands (type-check, lint:strict, tests) and include changes in a separate PR with thorough testing.
